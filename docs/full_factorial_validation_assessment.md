...
### Cadence provenance correction

The `original_lcsc` and `reference_lcsc` labels describe the intended
configurations, not verified short-cadence processing. The archived runners
for all 18 LC+SC arms omitted ALDERAAN's required `--use_sc True` argument,
and their logs report no short-cadence data. Their paired results cannot test
or rule out cadence. The remaining limb-darkening, printed-prior, and
repeat-seed comparisons retain their own target-panel provenance. A valid
cadence test is specified in
...
| `original_lcsc` | 9 | Intended LC+SC arm; archived output processed no verified short cadence |
| `reference_lcsc` | 9 | Intended LC+SC arm; archived output processed no verified short cadence |
...
| Intended LC+SC minus LC, original LD | 13 / 9 | +0.00101 | -0.00059 to +0.01280 | 0.00682 | Historical contrast; not a cadence test |
| Intended LC+SC minus LC, reference LD | 13 / 9 | -0.00081 | -0.04771 to +0.00263 | 0.00431 | Historical contrast; not a cadence test |
...
- `K00283` has the largest historical difference between the reference-LD
  intended-LC+SC and LC labels, with a maximum absolute planet-level
  eccentricity shift of 0.194. This is not evidence of a cadence response.
- `K02533` is the next largest system under that invalid contrast and has
  large duration changes.
...
1. Do not claim that the archived matrix tested or ruled out cadence. It does
   show that the separately scoped limb-darkening, printed-prior, and
   nested-sampling comparisons do not resolve the current Table 3 discrepancy.
...
4. Prioritize resolving the Berger et al. (2018) density construction and the
   ...
   public article.
5. A future corrected short-cadence validation must follow
   [`corrected_cadence_validation_plan.md`](corrected_cadence_validation_plan.md)
   before cadence is interpreted. The historical combined confirmation is a
   narrow configuration record, not a replacement cadence or population study.
