...
`metadata/public_reconstruction_20260727/README.md` for current numbers. The
final real-data control, residual attribution, full-sample post-fit sensitivity,
and nested-weighting audit are generated by:

```powershell
python scripts/gilbert_real_alderaan_control.py --help
python scripts/sagear_residual_attribution.py --help
python scripts/sagear_full_postfit_qc_sensitivity.py --help
python scripts/gilbert_prefit_cut_attribution.py --help
python scripts/nested_weighting_forensic_audit.py --help
```

Their compact committed outputs are under
`metadata/final_forensic_20260808/`. The commands require the excluded local
posterior ledgers and source catalogs; pass explicit paths when reproducing
them outside the original workspace.
