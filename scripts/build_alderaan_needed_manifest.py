...
def write_needed_scripts(cfg: dict, targets: pd.DataFrame, catalog_name: str, target_name: str) -> None:
    ...
    if repo and (repo / "bin" / "get_kepler_data.py").exists():
        lines += [
            ...
            "        python $AlderaanGet $kepid -c $Cadence -t lightcurve -o \"get_${kepid}_lc.sh\" --cmdtype curl",
    ...
    if repo and repo.exists():
        lines += [
            ...
            f"                python bin/analyze_autocorrelated_noise.py --mission {mission} --target $target --run_id $RunId --project_dir $ProjectDir *>> $log",
            "            }",
            "            if ($Phase -in @('all','fit')) {",
    ...
    download_sh.write_text(
        "\n".join(
            [
                ...
                '  python "$ALDERAAN_REPO/bin/get_kepler_data.py" "$kepid" -c "$CADENCE" -t lightcurve -o "get_${kepid}_lc.sh" --cmdtype curl',
                '  bash "get_${kepid}_lc.sh"',
                'done < "$TMP_KEPIDS"',
                'rm -f "$TMP_KEPIDS"',
            ]
    ...
    run_sh.write_text(
        "\n".join(
            [
                ...
                '    python bin/analyze_autocorrelated_noise.py --mission "$MISSION" --target "$target" --run_id "$RUN_ID" --project_dir "$PROJECT_DIR" >> "$log" 2>&1',
                "  fi",
