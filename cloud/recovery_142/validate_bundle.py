...
def main() -> None:
    ...
    parser.add_argument("--manifest", default="bundle_manifest_sha256.csv")
    ...
    if manifest_path.exists():
        ...
        required_manifest = {"file", "bytes", "sha256"}
        if not required_manifest.issubset(manifest.columns):
            failures.append("bundle manifest is missing file, bytes, or sha256")
        else:
            for _, row in manifest.iterrows():
                path = manifest_path.parent / str(row["file"])
                if not path.is_file():
                    failures.append(f"manifest file is missing: {path.name}")
                    continue
                if path.stat().st_size != int(row["bytes"]):
                    failures.append(f"manifest byte count mismatch: {path.name}")
                if sha256(path).lower() != str(row["sha256"]).lower():
                    failures.append(f"manifest SHA256 mismatch: {path.name}")
    if failures:
