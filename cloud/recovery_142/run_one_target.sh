...
TARGET_LOG_DIR="$BUNDLE_DIR/logs/targets/$RUN_ID"
STAGE="initialization"

mkdir -p "$DATA_DIR" "$STATUS_DIR" "$TARGET_LOG_DIR" "$BUNDLE_DIR/provenance"

# Keep a durable, target-specific record. GNU Parallel's aggregate output is
# not sufficient to identify which concurrent target produced a traceback.
exec >>"$TARGET_LOG_DIR/$TARGET.stdout.log" 2>>"$TARGET_LOG_DIR/$TARGET.stderr.log"
...
on_exit() {
  ...
  if [ "$rc" -ne 0 ] && [ "$state" = "running" ]; then
    printf 'failed_%s_exit_%s\n' "$STAGE" "$rc" > "$STATUS_FILE"
    printf '[%s] %s failed during %s with exit code %s\n' "$(date -Is)" "$TARGET" "$STAGE" "$rc" >&2
  fi
}
...
on_signal() {
  ...
  printf '[%s] %s interrupted by %s\n' "$(date -Is)" "$TARGET" "$signal" >&2
  exit "$rc"
}
...
echo "[$(date -Is)] Starting $TARGET / KIC $KEPID / seed offset $SEED_OFFSET"
...
if [ "$LC_COUNT" -eq 0 ]; then
  ...
  echo "missing_lightcurve" > "$STATUS_FILE"
  exit 4
fi
...
if [ -s "$RESULTS_FITS" ]; then
  ...
  echo "missing_results" > "$STATUS_FILE"
