...
BASE_PYTHON="$HOME/miniforge3/bin/python"
...
"$BASE_PYTHON" "$PWD/patch_alderaan_repro.py" "$ALDERAAN_REPO"
...
"$BASE_PYTHON" "$PWD/patch_alderaan_repro.py" "$ALDERAAN_PAPER_PRIOR_REPO"
if [ -f "$PWD/patch_alderaan_paper_priors.py" ]; then
  "$BASE_PYTHON" "$PWD/patch_alderaan_paper_priors.py" "$ALDERAAN_PAPER_PRIOR_REPO"
fi
...
echo "Run through bootstrap_shard.sh so the tested shard and job settings are used."
