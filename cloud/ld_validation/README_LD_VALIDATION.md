...
Recommended sequence on the GCP VM:
...
The runner is resumable. A valid `*-results.fits` is never overwritten; a
later invocation queues only targets still missing that file. Each target also
writes append-only logs under `logs/targets/<run_id>/`, and each arm writes
`projects/<arm>/status_manifest_<run_id>.csv`. An arm is complete only when
every target in its input CSV has a nonempty result FITS. A nonzero matrix exit
means one or more arms remain incomplete; it does not erase successful fits.

Before a long resume, increase the VM automatic-stop window from the original
24 hours to 48 hours in Cloud Shell:
